/**
 * Windows variant of `Button.tsx`. Identical except:
 *
 * - Its underlying touchable is React Native core's `Pressable` — driving
 *   its own `onPressIn`/`onPressOut`-triggered opacity dip via a plain,
 *   JS-driven `Animated.Value` — instead of `@gorhom/bottom-sheet`'s
 *   `TouchableOpacity`, which pulls in Reanimated/RNGH machinery Windows
 *   doesn't otherwise need.
 * - The gradient fill renders through RNW's own native SVG support (RN core
 *   `Image` recognizing an SVG payload, via WinUI's `SvgImageSource`)
 *   instead of `react-native-svg`'s `Svg`/`Defs`/`LinearGradient`/`Rect` —
 *   see `ButtonGradientFill`'s own doc comment and `Icon.windows.tsx`'s
 *   header comment for why `react-native-svg` itself isn't an option here.
 *
 * @module @noteferry/ui/Primitive/Button
 *
 * @file      Button.windows.tsx
...
import { EncodeSvgDataUri, Mix, WithAlpha } from "../Utility/index.js";
...
/**
 * Builds a standalone, aspect-ratio-agnostic two-stop gradient SVG string —
 * \`preserveAspectRatio="none"\` so it stretches to fill exactly, matching
 * the mobile file's \`react-native-svg\` \`<Rect width="100%" height="100%">\`
 * (which does the same regardless of the button's own aspect ratio).
 */
const BuildGradientMarkup = (Top: string, Bottom: string): string =>
    "<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 100 100\" preserveAspectRatio=\"none\">"
    + "<defs><linearGradient id=\"Fill\" x1=\"0\" y1=\"0\" x2=\"0\" y2=\"1\">"
    + `<stop offset="0%" stop-color="${ Top }"/>`
    + `<stop offset="100%" stop-color="${ Bottom }"/>`
    + "</linearGradient></defs>"
    + "<rect width=\"100\" height=\"100\" fill=\"url(#Fill)\"/></svg>";
 ...
 * so an absolutely-positioned, corner-matched `Image` renders as the first
 * child of the `Button`, behind its label/icon — through RNW's own native
 * SVG support (RN core `Image` recognizing an SVG payload via WinUI's
 * `SvgImageSource`) instead of `react-native-svg`'s `Svg`/`Defs`/
 * `LinearGradient`/`Rect`, since that library's own Windows native module
 * doesn't currently build on this app's Fabric/New-Architecture toolchain —
 * see `Icon.windows.tsx`'s header comment for the full story.
 *
 * @category Component
 * @since 1.0.0
 */
...
{
    const Uri = React.useMemo(() => EncodeSvgDataUri(BuildGradientMarkup(Top, Bottom)), [ Top, Bottom ]);
...
}: ButtonProps, ForwardedRef: React.ForwardedRef<View>): React.JSX.Element =>
{
    ...
    /* RN core's `Pressable` has no built-in `TouchableOpacity`-style press
     * dimming, so it's reimplemented here as a small, JS-driven
     * `Animated.Value` — no Reanimated/worklets involved. */
    const OpacityAnim = React.useRef(new Animated.Value(1)).current;

    const onPressIn = () =>
    {
        SetIsPressed(true);
        Animated.timing(OpacityAnim, { duration: 100, toValue: 0.7, useNativeDriver: true }).start();
    };

    const onPressOut = () =>
    {
        SetIsPressed(false);
        Animated.timing(OpacityAnim, { duration: 100, toValue: 1, useNativeDriver: true }).start();
    };
    ...
    return (
        <Pressable
            ...
            <Animated.View style={ [ Styles.OpacityLayer, { opacity: OpacityAnim } ] }>
...
const useStyles = MakeStyles({
    ...
    Label: TextStyle({
        /* Fill the row so `textAlign` positions the glyph. */
        flexGrow: 1,
        fontSize: 12,
        lineHeight: 22
    }),
    OpacityLayer: MakeViewStyle({
        alignItems: "center",
        flexDirection: "row",
        flexGrow: 1,
        justifyContent: "center"
