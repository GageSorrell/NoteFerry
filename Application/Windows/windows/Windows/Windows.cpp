...
namespace
{
// Formats the packaged app's version (from Package.appxmanifest) for display
...
_Use_decl_annotations_ int CALLBACK WinMain(HINSTANCE instance, HINSTANCE, PSTR /* commandLine */, int showCmd)
{
    ...
    appWindow.Title(L"NoteFerryWin");
    appWindow.Resize({1000, 1000});

    // Add a system tray icon. Right-clicking it shows a context menu with the
    // app's name/version (disabled), Settings, and Exit.
    HWND mainWindow{winrt::Microsoft::UI::GetWindowFromWindowId(appWindow.Id())};
    NoteFerry::TrayIcon trayIcon(mainWindow, instance, GetAppVersionString());
    trayIcon.OnSettingsRequested = [instance] { NoteFerry::SettingsWindow::ShowOrActivate(instance); };
    trayIcon.OnExitRequested = [&appWindow] { appWindow.Destroy(); };

    // Get the ReactViewOptions so we can set the initial RN component to load
    auto viewOptions{reactNativeWin32App.ReactViewOptions()};
    viewOptions.ComponentName(L"NoteFerryWin");
